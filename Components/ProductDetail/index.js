...
// const ProductDetail = ({ data ,params}) => {
...
//   } = useSWR(
//     `${process.env.NEXT_PUBLIC_API}/v3/products/related/${params}`,
//     fetcher
//   );
//   const {
...
//   } = useSWR(
//     `${process.env.NEXT_PUBLIC_API}/v3/products/related_item/${params}`,
//     fetcher
//   );
...
//   } = useSWR(
//     `${process.env.NEXT_PUBLIC_API}/v3/recent-products/${params}`,
//     fetcher
//   );
...
import { singleCardData } from "@/data";
const ProductDetail = () => {
  // console.log("Single", singleCardData);
  const [brandSelectSize, setBrandSelectSize] = useState(1);
  const [sortSelectSize, setSortSelectSize] = useState(1);
  const [activeTab, setActiveTab] = useState("description");
  const [isAutoPlay, setIsAutoPlay] = useState(true);
        ...
        <div className="flex flex-col lg:flex-row justify-between bg-white">
          ...
          <div className="lg:w-[55%] p-2 md:ml-8  md:p-8 rounded-lg">
            ...
            <div className="mb-4 text-[#0ABB75]">
              <span className=" text-black">Category:</span> Bluetooth Module
            </div>
            <div className="mb-4 text-[#0ABB75]">
              <span className=" text-black">SKU:</span> 1963
            </div>
            <hr className="mb-4" />
            <div className="mb-4">
            ...
            <div className="text-lg mb-4 text-black">
              Discounted Price:
              <span className="text-red-600 text-2xl ">₹140 /pc:</span>
            </div>
            <hr className="mb-4" />
            <div className="mb-4 flex flex-col md:flex-row items-center gap-5">
              ...
              <div className="flex items-center mt-2">
                ...
                <input
                  type="text"
                  className="border w-12 text-center mx-2 text-white"
                  value="1"
                />
                <button className="border w-10 h-10 rounded-full text-[#797D81] bg-[#E2E6EA]">
                  +
                </button>
                <span className="ml-2 text-gray-500">(20 available)</span>
              </div>
            </div>
            <hr className="mb-4" />

            <div className="mb-4 text-black">
              <span className="text-black">Weight:</span> 40g
            </div>
            <hr className="mb-4" />
            <div className="text-2xl mb-4 text-black">
              Total Price: <span className="text-primary-red">₹140</span>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-10 items-center">
              <button className="bg-gray-800  text-white px-12 lg:px-4 py-2 rounded-md hover:bg-gray-900 flex justify-center text-nowrap items-center gap-1">
                <div className="flex items-center gap-1">
                  <AiOutlineShopping />
                  <p>Add to cart</p>
                </div>
      ...
      <section className="flex text-black w-full">
        ...
        <aside className="w-[27%] p-4 bg-defaultBg px-4 shadow h-screen overflow-y-auto hidden lg:block">
          ...
          <div className="w-full mt-6 pb-2 px-4  bg-white shadow-lg">
            <p className="py-2 font-semibold ">Top Selling Products</p>
            <hr className="py-2 pb-4" />
            <div className="w-full border-b shadow-sm px-1 flex justify-between items-center ">
              <Image
                src={"/Images/productdetailtopsale.png"}
                width={100}
                height={100}
                alt="Arduino Uno Protoshield"
                className="w-full h-32 object-contain mb-4 "
              />
              <div className="">
                <h2 className="text-sm  mb-2 text-[#5E5E67]">
                  Arduino Uno Protoshield + Mini Breadboard
                </h2>
                <div className="flex items-center mb-2">
                ...
                <div className=" flex gap-2 items-center ">
                  ...
                  <p className="text-md font-bold text-red-500 ">₹106</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="w-full lg:w-[73%] p-4 bg-defaultBg h-screen  overflow-y-scroll">
              ...
              <div className="bg-white">
                ...
                <div className="p-6 bg-white text-medium  rounded-lg ">
                  {activeTab === "description" && (
                    <div className="w-96">
                      <p>
                        1 Chipset :ESPRESSIF-ESP32 240MHz Xtensa®
                        single-/dual-core 32-bit LX6 microprocessor 2 FLASH:
                        QSPI flash 4MB / PSRAM 8MB 3 SRAM:520 kB SRAM 4 Button:
                        Reset 5 USB to TTL: CP2104 Description This is a TTGO
                        T-Call V1.4 ESP32 Wireless Module SIM Antenna SIM Card
                        SIM800L Module Unsoldered. The module is used for
                        wireless communication. The ESP V1.4 ESP32 Serial WIFI
                        Wireless Transceiver Module is a self-contained SOC with
                        an integrated TCP/IP protocol stack that can give any
                        microcontroller access to your Wi-Fi network. This
                        module has a powerful enough onboard processing and
                        storage capability that allows it to be integrated with
                        the sensors and other application-specific devices
                        through its GPIOs with minimal development up-front and
          ...
          <div className=" bg-white mt-4 ">
            ...
            <div className="px-2 pb-4">
              <Slider slidepre={4}>
                <div className="flex">
                  {singleCardData.map((cardData, index) => (
